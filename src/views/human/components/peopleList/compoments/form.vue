  ...
  <div class="form">
    ...
    <el-dialog title="新增人员" :visible="dialogTableVisible">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click.native="cancelFn" style="margin-right: 30px"
          >取 消</el-button
        >
        <el-button type="primary" @click.native="addFnN">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 新增弹窗 -->
...
export default {
  ...
  methods: {
    ...
    // 点击确定发送 添加 
   async addFnN() {
       let regionIndex = this.quyuList.findIndex((item) => {
          return item.id === this.form.regionId
        })
        let regionName = this.quyuList[regionIndex].name
        console.log(regionName)
     const res =  await addUserInfoApi({
    ...
    amend(val) {
      ...
      this.form.regionName = val.regionName;
      this.form.regionId = val.regionId;
    ...
    // 点击确定发送编辑
    async confirmFn() {
      const res = await editUserInfoApi(this.id, {
        userName: this.form.userName,
        roleId: this.form.role.roleId,
        mobile: this.form.mobile,
        regionId: this.form.regionId,
